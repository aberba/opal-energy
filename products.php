...
<?php
         foreach($Product_results as $product => $value) {
            ...
            $image_file = "/uploads/products/".$value->image_one; 
...
if($Pagination->total_pages() > 1) {
   echo "<section class='pagination-section clearfix'>";
    ...
    if($Pagination->has_previous_page()) {
        echo " <a class='previous' href='/products.php?category={$cat_url_name}&sort={$sort}&page=";
   ...
   if($Pagination->has_next_page()) {
        echo " <a class='next' href='/products.php?category={$cat_url_name}&sort={$sort}&page=";
